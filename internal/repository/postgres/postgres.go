...
func (r *PostgresUserRepo) CreateUser(ctx context.Context, u domain.UserInput) error {
	id := uuid.New()
	_, err := r.db.Exec(`INSERT INTO users (id, firstname, lastname, age) VALUES ($1, $2, $3, $4)`, id, u.Firstname, u.Lastname, u.Age)
	return err
}

func (r *PostgresUserRepo) GetAll(ctx context.Context) ([]domain.User, error) {
	rows, err := r.db.Query(`SELECT id, firstname, lastname, age FROM users`)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
...
func (r *PostgresUserRepo) GetByID(ctx context.Context, id uuid.UUID) (domain.User, error) {
	...
	err := r.db.QueryRowContext(ctx, "SELECT id, firstname, lastname, age FROM users WHERE id = $1", id).Scan(&user.ID, &user.Firstname, &user.Lastname, &user.Age)
	return user, err
}
